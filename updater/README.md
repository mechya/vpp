...
The updater is the `vpp_updater` library, used by the viewer. There is no separate tool.
...
```text
build\viewer\vpp_viewer.exe https://example.com/hello/home.vpp
```
...
On Windows the updater uses WinHTTP with the system proxy settings and short timeouts, so an unreachable server fails fast and the installed copy takes over. Other platforms return an error until a client is added; libcurl is the likely choice.
