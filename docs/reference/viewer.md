...
> **Reference specification.** This page describes how the C++ viewer behaved before it was removed (last present in commit `1d1cd11`; read its source with `git show 1d1cd11:viewer/src/main.cpp`). The Rust port in `crates/vpp-viewer` reproduces this behaviour unless `docs/rust-port.md` says otherwise. Commands use the tool names the Rust port keeps (`vppc`, `vpppack`, `vpp-viewer`); they work once that crate is ported.
...
```text
vppc examples\hello-world\index.html
vpppack examples\hello-world --key examples\hello-world\publisher.key
vpp-viewer examples\hello-world\hello-world.vpp
```
