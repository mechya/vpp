...
The packager is the `vpppack` tool, built into `build\packager\vpppack.exe`.
...
```text
build\packager\vpppack.exe --inspect examples\hello-world\out\home.vpp
build\packager\vpppack.exe --inspect examples\hello-world\out\home.vppm
```
...
```text
VPPK, format version 2
manifest      site id, name, version, page         \  signed region
resources     for each: name, offset, size, SHA-256 /
publisher key 32 bytes, or empty
signature     64 bytes Ed25519, or empty
data          the resources, back to back
```

All integers are little-endian. Strings are length-prefixed UTF-8. Resource names are unique and may contain directories (`style/00-global.bin`). A `.vppm` file is the signed region plus the key and signature, without the data.

The signature covers the manifest and the entry table. Because the table holds every resource's hash, the signature transitively covers the data as well. Changing one byte anywhere in the file either breaks a resource hash or breaks the signature.
